...
class LockFreeQueue {
  ...
  static constexpr std::size_t const Limit = StepPrime * Capacity / 2;
  ...
  std::atomic<T*>* _ring;
  std::atomic<std::size_t> _head;   // we have _head <= _tail at all times
  char padding[56];
  std::atomic<std::size_t> _tail;   // _head == _tail is empty
  char padding2[56];
  std::size_t _nrInQueueOnPop;
  std::size_t _nrPops;

 public:

  LockFreeQueue() : _head(0), _tail(0), _nrInQueueOnPop(0), _nrPops(0) {
  ...
  bool try_push(T* p) {
    ...
    for (;;) {
      T* expected = nullptr; 
      if (_ring[pos].compare_exchange_weak(
            expected, p, std::memory_order_relaxed)) {
        return true;
      }
    }
  }
  ...
  bool try_pop(T*& result) {
    ...
    _nrInQueueOnPop += tail - head;
    _head.fetch_add(StepPrime, std::memory_order_relaxed);
    std::size_t pos = head & CapMask;
    for (;;) {
      T* res = _ring[pos].exchange(nullptr, std::memory_order_relaxed);
      if (res != nullptr) {
        result = res;
        return true;
      }
  ...
  double avgInQueue() const {
    return (double) _nrInQueueOnPop / (_nrPops * StepPrime);
  }

  std::size_t nrPops() const {
    return _nrPops;
  }
};
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Total time: " <<  nanoseconds << " ns for "
    ...
    << " ns/item" << std::endl;
#ifdef MEINE
  std::cout << "Stats: " << q->avgInQueue() << " " << q->nrPops() << std::endl;
#endif
  return 0;
}
